...
router.post('/', [
    express_validator_1.body('creator').not().isEmpty(),
    express_validator_1.body('name').isLength({ min: 3, max: 30 })
...
router.post('/:groupId/fork', [
    express_validator_1.body('userId').isLength({ min: 24, max: 24 }),
    express_validator_1.body('postId').isLength({ min: 24, max: 24 })
